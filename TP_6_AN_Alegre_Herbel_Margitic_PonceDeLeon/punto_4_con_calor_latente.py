...
''' Este codigo realiza el punto 4 considerando el calor latente de fusion (VERSION FINAL)'''
''' ESTA ES LA VERSION FINAL Y LOS RESULTADOS EN EL INFORME CORRESPONDEN A ESTE CODIGO.'''
...
# funcion que se utiliza para actualizar la temperatura y la energia en cada paso (IGUAL QUE EN EL PUNTO 2)
@jit(nopython=True)
    ...
    for i in range(1, Nx-1):
        for j in range(1, Ny-1):
            if mask[i, j]:
                ...
                # calcular energia (entalpia) y temperatura
...
# funcion que se utiliza para actualizar la temperatura y la energia en cada paso (NUEVO METODO, SE CONSIDERA CADA FASE EXPLICITAMENTE)
@jit(nopython=True)
...
def simular_cambio_fase():
    ...
    for step in range(steps):
        #Usamos la vieja porque parece andar mejor, pero ambas dan resultados similares
        temp, energy, ice_fraction = calcular_paso_vieja(temp, energy, mask, Dx, Dy, Dt, TEMP_MELT, CALOR_LATENTE_FUSION)
        
        # medicion la fracción de hielo en el corazón
        ...
        if 0: #grafica la T a traves del tiempo (lo hace mejor el otro codigo)
            temp_masked = np.ma.masked_array(temp, ~mask)
