...
rho_ice = 917  # Densidad del hielo (kg/m^3)
rho_water = 997  # Densidad del agua (kg/m^3)
c_ice = 2100  # Capacidad calorífica del hielo (J/kg*K)
...
H = np.where(T < aKelvin(0), rho_ice * c_ice * aKelvin(-10), rho_water * c_water * aKelvin(0))
...
def plot_temperature_line(T, t, x_length, y_length):
    middle_y = ny // 2  # Línea central en y
    temperature_profile = T[:, middle_y]  # Tomar el perfil de temperatura en la línea central
...
for n in range(nt):
    ...
    # Actualización del campo de temperatura
    for i in range(1, nx - 1):
        for j in range(1, ny - 1):
            # Selección de propiedades térmicas
            if T_old[i, j] < aKelvin(0):  # Hielo
                alpha, c, rho = alpha_ice, c_ice, rho_ice
            else:  # Agua
                alpha, c, rho = alpha_water, c_water, rho_water
            ...
            if H[i, j] < rho_ice * c_ice * aKelvin(0):  # Sólido (hielo)
                T[i, j] = H[i, j] / (rho_ice * c_ice)
            elif H[i, j] > rho_water * c_water * aKelvin(0):  # Líquido (agua)
                T[i, j] = H[i, j] / (rho_water * c_water)
            else:  # Cambio de fase
                T[i, j] = aKelvin(0)

    # Aplicar condiciones de frontera
    T[0, :] = T[1, :]  # Borde izquierdo aislado
    T[-1, :] = boundary_condition_right(t)  # Borde derecho con temperatura cambiante
    T[:, 0] = T[:, 1]  # Borde inferior aislado
    T[:, -1] = T[:, -2]  # Borde superior aislado

    # Graficar la temperatura a lo largo de la línea central (cada 10 segundos)
    if n % int(10 / dt) == 0:  # Graficar cada 10 segundos
        plot_temperature_line(T, t, x_length, y_length)

# Ajustar el gráfico
plt.xlabel("Distancia (mm)")
plt.ylabel("Temperatura (°C)")
plt.title("Distribución de temperatura a lo largo de la línea central")
plt.legend()
plt.grid(True)
plt.show()
